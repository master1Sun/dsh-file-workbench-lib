...
function apply(ctx) {
  ...
  if (!document.querySelector(`link[href="${apiBase}/${"assets/style-CCMxMHov.css"}"]`)) {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = `${apiBase}/${"assets/style-CCMxMHov.css"}`;
    document.head.appendChild(link);
  }
  if (!document.querySelector(`script[src="${apiBase}/${"assets/index-CS4iyBUB.js"}"]`)) {
    const script = document.createElement("script");
    script.type = "module";
    script.src = `${apiBase}/${"assets/index-CS4iyBUB.js"}`;
