...
function tryMountVs(el) {
  const mfn = window.__dshVSCodeMountPane__;
  if (typeof mfn === "function") {
    return mfn(el, { apiBase: `${window.location.origin}${API_PREFIX}` });
  }
  return null;
}
function VSCodePaneBridge() {
  const ref = (0, import_react.useRef)(null);
  (0, import_react.useEffect)(() => {
    const el = ref.current;
    if (!el) return;
...
function apply(ctx) {
  ...
  if (!document.querySelector(`link[href="${apiBase}/${"assets/style-gGotV657.css"}"]`)) {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = `${apiBase}/${"assets/style-gGotV657.css"}`;
    document.head.appendChild(link);
  }
  if (!document.querySelector(`script[src="${apiBase}/${"assets/index-D3B8Evg8.js"}"]`)) {
    const script = document.createElement("script");
    script.type = "module";
    script.src = `${apiBase}/${"assets/index-D3B8Evg8.js"}`;
