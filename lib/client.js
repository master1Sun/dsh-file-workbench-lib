...
// test/activity-bar-injection.js
(() => {
  function install(attempt = 0) {
    const workbenchApi = window.__dshFileWorkbenchWorkbench__;
    const editorApi = window.__dshFileWorkbenchVSCode__;
    if (!workbenchApi && !editorApi && attempt < 100) {
      window.setTimeout(() => install(attempt + 1), 100);
      return;
    }
    if (!workbenchApi) console.warn("[dsh-slot-test] \u5DE5\u4F5C\u53F0 API \u672A\u5C31\u7EEA");
    if (!editorApi) console.warn("[dsh-slot-test] \u7F16\u8F91\u5668 API \u672A\u5C31\u7EEA");
    if (!workbenchApi && !editorApi) return;
    const styleText = `
    .dsh-slot-demo{height:100%;box-sizing:border-box;padding:16px;overflow:auto;color:var(--dsh-fg,#c9d1d9);font:13px var(--dsh-font,sans-serif)}
    .dsh-slot-demo *{box-sizing:border-box}.dsh-slot-head{display:flex;justify-content:space-between;gap:12px;margin-bottom:16px}
    .dsh-slot-title{font-size:18px;font-weight:650}.dsh-slot-sub{margin-top:4px;color:var(--dsh-fg-weak,#8b949e);font-size:12px}
    .dsh-slot-badge{padding:4px 8px;border:1px solid var(--dsh-border,#30363d);border-radius:999px;color:var(--dsh-accent,#2f81f7);white-space:nowrap}
    .dsh-slot-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-bottom:14px}
    .dsh-slot-card,.dsh-slot-row{padding:10px;border:1px solid var(--dsh-border,#30363d);border-radius:6px;background:var(--dsh-bg2,#161b22)}
    .dsh-slot-label{margin-bottom:5px;color:var(--dsh-fg-weak,#8b949e);font-size:11px}.dsh-slot-value{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .dsh-slot-actions{display:flex;flex-wrap:wrap;gap:7px;margin:14px 0}.dsh-slot-list{display:flex;flex-direction:column;gap:6px;margin-top:8px}
    .dsh-slot-row{display:flex;align-items:center;justify-content:space-between;gap:8px}.dsh-slot-path{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .dsh-slot-btn{padding:6px 10px;border:1px solid var(--dsh-border,#30363d);border-radius:5px;color:var(--dsh-fg,#c9d1d9);background:var(--dsh-bg2,#161b22);cursor:pointer}
    .dsh-slot-btn:hover{border-color:var(--dsh-accent,#2f81f7);background:var(--dsh-hover,#30363d)}.dsh-slot-btn.primary{border-color:var(--dsh-accent,#2f81f7)}
    .dsh-slot-log{min-height:38px;margin-top:14px;padding:9px;border-left:3px solid var(--dsh-accent,#2f81f7);background:var(--dsh-bg2,#161b22);color:var(--dsh-fg-weak,#8b949e)}
  `;
    function addStyle(el) {
      const style = document.createElement("style");
      style.dataset.dshSlotTest = "";
      style.textContent = styleText;
      el.appendChild(style);
    }
    function button(label, handler, primary = false) {
      const el = document.createElement("button");
      el.className = `dsh-slot-btn${primary ? " primary" : ""}`;
    ...
    function card(label, value) {
      ...
      el.className = "dsh-slot-card";
      el.innerHTML = '<div class="dsh-slot-label"></div><div class="dsh-slot-value"></div>';
      el.firstElementChild.textContent = label;
      el.lastElementChild.textContent = value || "\u672A\u8BBE\u7F6E";
      return el;
    }
    function addFileRow(list, name, openFile) {
      const row = document.createElement("div");
      row.className = "dsh-slot-row";
      const path = document.createElement("span");
      path.className = "dsh-slot-path";
      path.textContent = name;
      row.append(path, button("\u6253\u5F00", () => openFile(name)));
      list.appendChild(row);
    }
    function mountDemo(el, ctx, mode) {
      el.replaceChildren();
      addStyle(el);
      const isWorkbench = mode === "workbench";
      const title = isWorkbench ? "\u9879\u76EE\u5DE5\u4F5C\u53F0\u63D2\u4EF6" : "\u4EE3\u7801\u5BA1\u67E5\u63D2\u4EF6";
      const files = ["README.md", "package.json", "src/index.ts", "src/components/App.vue"];
      const root = document.createElement("div");
      root.className = "dsh-slot-demo";
      root.innerHTML = '<div class="dsh-slot-head"><div><div class="dsh-slot-title"></div><div class="dsh-slot-sub"></div></div><div class="dsh-slot-badge"></div></div><div class="dsh-slot-grid"></div><div class="dsh-slot-actions"></div><div class="dsh-slot-section"></div><div class="dsh-slot-list"></div><div class="dsh-slot-log">\u7B49\u5F85\u64CD\u4F5C...</div>';
      root.querySelector(".dsh-slot-title").textContent = title;
      root.querySelector(".dsh-slot-sub").textContent = isWorkbench ? "\u9879\u76EE\u6982\u89C8\u3001\u6700\u8FD1\u6587\u4EF6\u548C\u5DE5\u4F5C\u533A\u64CD\u4F5C" : "\u6587\u4EF6\u6811\u3001\u4EE3\u7801\u6253\u5F00\u548C\u53D8\u66F4\u5BA1\u67E5\u64CD\u4F5C";
      root.querySelector(".dsh-slot-badge").textContent = isWorkbench ? "\u5DE5\u4F5C\u53F0 API" : "\u7F16\u8F91\u5668 API";
      el.appendChild(root);
      const grid = root.querySelector(".dsh-slot-grid");
      const actions = root.querySelector(".dsh-slot-actions");
      const list = root.querySelector(".dsh-slot-list");
      const log = root.querySelector(".dsh-slot-log");
      const writeLog = (message) => {
        log.textContent = `${(/* @__PURE__ */ new Date()).toLocaleTimeString()}  ${message}`;
      };
      const renderContext = () => grid.replaceChildren(card("\u9879\u76EE\u76EE\u5F55", ctx.projectDir || "\u5F53\u524D\u6CA1\u6709\u6253\u5F00\u9879\u76EE"), card("\u4E3B\u9898", ctx.theme), card("API \u7248\u672C", String(ctx.apiVersion)));
      const pathOf = (name) => ctx.projectDir ? `${ctx.projectDir.replace(/[\\/]$/, "")}/${name}` : name;
      const openFile = (name) => {
        if (!ctx.projectDir) {
          ctx.toast("info", "\u8BF7\u5148\u6253\u5F00\u9879\u76EE\u76EE\u5F55");
          writeLog("\u6253\u5F00\u6587\u4EF6\u5931\u8D25\uFF1A\u6CA1\u6709\u9879\u76EE\u76EE\u5F55");
          return;
        }
        void ctx.openFile(pathOf(name));
        ctx.toast("ok", `\u5DF2\u8BF7\u6C42\u6253\u5F00 ${name}`);
        writeLog(`ctx.openFile -> ${pathOf(name)}`);
      };
      const showDiff = () => {
        ctx.openDiff("\u63D2\u4EF6\u793A\u4F8B\u53D8\u66F4", ["@@ -1,3 +1,5 @@", " const enabled = true;", "+const source = 'activity-slot';", "+console.log(source);"]);
        ctx.toast("info", "\u5DF2\u8C03\u7528 ctx.openDiff");
        writeLog("ctx.openDiff -> \u63D2\u4EF6\u793A\u4F8B\u53D8\u66F4");
      };
      actions.append(button("\u5237\u65B0\u4E0A\u4E0B\u6587", () => {
        renderContext();
        writeLog("\u5DF2\u5237\u65B0\u4E0A\u4E0B\u6587");
      }, true), button("\u53D1\u9001\u6D4B\u8BD5 Toast", () => {
        ctx.toast("ok", `${title} \u63A5\u5165\u6B63\u5E38`);
        writeLog("ctx.toast -> ok");
      }));
      if (isWorkbench) {
        actions.append(button("\u6253\u5F00 README.md", () => openFile("README.md"), true));
        root.querySelector(".dsh-slot-section").textContent = "\u6700\u8FD1\u8BBF\u95EE\u6587\u4EF6";
        files.slice(0, 3).forEach((name) => addFileRow(list, name, openFile));
      } else {
        actions.append(button("\u6253\u5F00\u5165\u53E3\u6587\u4EF6", () => openFile("src/index.ts"), true), button("\u67E5\u770B\u793A\u4F8B Diff", showDiff));
        root.querySelector(".dsh-slot-section").textContent = "\u9879\u76EE\u6587\u4EF6\u6811\uFF08\u6A21\u62DF\uFF09";
        files.forEach((name) => addFileRow(list, name, openFile));
      }
      renderContext();
      const offProject = ctx.onProjectChange((projectDir) => {
        renderContext();
        writeLog(`\u9879\u76EE\u76EE\u5F55\u53D8\u5316 -> ${projectDir || "(null)"}`);
      });
      const offTheme = ctx.onThemeChange((theme) => {
        renderContext();
        writeLog(`\u4E3B\u9898\u53D8\u5316 -> ${theme}`);
      });
      return () => {
    ...
    if (workbenchApi) {
      workbenchApi.activityBar.unregister("dsh-slot-test.workbench");
      workbenchApi.activityBar.register({
        id: "dsh-slot-test.workbench",
        title: { zh: "\u6D4B\u8BD5\u9879\u76EE\u5DE5\u4F5C\u53F0", en: "Project Workbench Test" },
        icon: "folder",
        order: 900,
        mount: (el, ctx) => mountDemo(el, ctx, "workbench")
      });
    }
    if (editorApi) {
      editorApi.activityBar.unregister("dsh-slot-test.editor");
      editorApi.activityBar.register({
        id: "dsh-slot-test.editor",
        title: { zh: "\u6D4B\u8BD5\u4EE3\u7801\u5BA1\u67E5", en: "Code Review Test" },
        icon: "code",
        order: 901,
        mount: (el, ctx) => mountDemo(el, ctx, "editor")
      });
    }
    console.info("[dsh-slot-test] \u5DF2\u5206\u522B\u6CE8\u518C\u5DE5\u4F5C\u53F0\u548C\u7F16\u8F91\u5668\u6D4B\u8BD5\u63D2\u69FD");
  }
  install();
})();
...
function apply(ctx) {
  ...
  if (!document.querySelector(`link[href="${apiBase2}/${"assets/style-CUtz3vWr.css"}"]`)) {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = `${apiBase2}/${"assets/style-CUtz3vWr.css"}`;
    document.head.appendChild(link);
  }
  if (!document.querySelector(`script[src="${apiBase2}/${"assets/index-Bf3sVCeh.js"}"]`)) {
    const script = document.createElement("script");
    script.type = "module";
    script.src = `${apiBase2}/${"assets/index-Bf3sVCeh.js"}`;
