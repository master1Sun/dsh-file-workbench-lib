...
var zh = {
  ...
  sortType: "\u7C7B\u578B",
  sortSize: "\u5927\u5C0F",
  sortRecycleDate: "\u5220\u9664\u65F6\u95F4",
  sortRecycleOrig: "\u539F\u4F4D\u7F6E",
  sortBy: "\u6392\u5E8F",
  sortAsc: "\u5347\u5E8F",
  sortDesc: "\u964D\u5E8F",
  recycleFootCount: "\u5171 {count} \u9879",
  recycleFootSize: "\u5408\u8BA1 {size}",
  recycleFootHint: "\u53F3\u952E\u53EF\u5207\u6362\u67E5\u770B/\u6392\u5E8F",
  recycleFootSortHint: "\u70B9\u51FB\u5207\u6362\u6392\u5E8F\u5B57\u6BB5",
  moved: "\u5DF2\u79FB\u52A8",
  copied: "\u5DF2\u590D\u5236",
  ...
  actHourAgo: "{n} \u5C0F\u65F6\u524D",
  actDayAgo: "{n} \u5929\u524D",
  actOpenFile: "\u70B9\u51FB\u5728\u5DE5\u4F5C\u53F0\u6253\u5F00",
  compatTitle: "DSH \u7248\u672C\u8FC7\u4F4E",
  compatBody: "AI \u5BF9\u8BDD\u3001\u4F1A\u8BDD\u4EFB\u52A1\u3001\u4F1A\u8BDD\u6D3B\u52A8\u3001\u5B50\u4EE3\u7406\u9762\u677F\u4EC5\u652F\u6301 DSH 0.1.2-rc.1 \u53CA\u4EE5\u4E0A\u7248\u672C\u3002\u5F53\u524D\u5BBF\u4E3B\u672A\u63D0\u4F9B\u4F1A\u8BDD\u4E8B\u4EF6\u63A5\u53E3\uFF0C\u8BF7\u5347\u7EA7 DSH \u540E\u91CD\u542F\u3002",
  taskTreeSession: "\u5F53\u524D\u4F1A\u8BDD",
  taskTreeGroupSession: "\u4F1A\u8BDD\u4EFB\u52A1",
  taskTreeGroupSubagent: "\u5B50\u4EE3\u7406",
  taskTreeGroupHistory: "\u5386\u53F2\u4F1A\u8BDD",
  taskTreeUnknownWs: "\u672A\u5173\u8054\u5DE5\u4F5C\u533A",
  taskHistWorkspace: "\u5DE5\u4F5C\u533A",
  taskHistLastActive: "\u6700\u8FD1\u6D3B\u8DC3",
  taskHistHint: "\u4EC5\u67E5\u770B\u4F1A\u8BDD\u4FE1\u606F\uFF0C\u4E0D\u4F1A\u5207\u6362\u5F53\u524D\u4F1A\u8BDD\u3002",
  taskHistConv: "\u4F1A\u8BDD\u5185\u5BB9",
  taskHistLoading: "\u52A0\u8F7D\u4E2D\u2026",
  taskHistEmptyConv: "\u6682\u65E0\u4F1A\u8BDD\u8BB0\u5F55\u3002",
  taskHistRoleSystem: "\u7CFB\u7EDF",
  gitMenu: "Git",
  gitAdd: "\u6682\u5B58 (git add)",
...
var en = {
  ...
  sortType: "Type",
  sortSize: "Size",
  sortRecycleDate: "Date deleted",
  sortRecycleOrig: "Original path",
  sortBy: "Sort by",
  sortAsc: "Ascending",
  sortDesc: "Descending",
  recycleFootCount: "{count} items",
  recycleFootSize: "Total {size}",
  recycleFootHint: "Right-click to switch view / sort",
  recycleFootSortHint: "Click to cycle sort field",
  moved: "Moved",
  copied: "Copied",
  ...
  actHourAgo: "{n} h ago",
  actDayAgo: "{n} d ago",
  actOpenFile: "Click to open in workbench",
  compatTitle: "DSH version too old",
  compatBody: "AI Chat, Session Tasks, Session Activity and Subagent panels require DSH 0.1.2-rc.1 or newer. The session event interface is unavailable on this host; please upgrade DSH and restart.",
  taskTreeSession: "Current session",
  taskTreeGroupSession: "Session tasks",
  taskTreeGroupSubagent: "Subagents",
  taskTreeGroupHistory: "History sessions",
  taskTreeUnknownWs: "No workspace",
  taskHistWorkspace: "Workspace",
  taskHistLastActive: "Last active",
  taskHistHint: "View only \u2014 the current session is not switched.",
  taskHistConv: "Conversation",
  taskHistLoading: "Loading\u2026",
  taskHistEmptyConv: "No conversation records.",
  taskHistRoleSystem: "System",
  gitMenu: "Git",
...
function apply(ctx) {
  ...
  const resolveMainBinding = () => {
    try {
      const id = getSessionId();
      if (!id) return void 0;
      return ctx.sessions?.binding?.(id);
    } catch {
      return void 0;
    }
  };
  ...
  const normalizeFromEvents = (sess, win, sessionIdFallback) => {
    const turns = [];
    const entries = Array.isArray(win?.entries) ? win.entries : [];
    ...
    for (const entry of entries) {
      ...
      if (etype === "user/message") {
        ...
        lastAssistantSeq = Math.max(lastAssistantSeq, seq);
        partialText = "";
        partialReasoning = "";
        ...
        for (const b of Array.isArray(msg.content) ? msg.content : []) {
          ...
          else if (blk.type === "reasoning") reasoning.push(blk.text ?? "");
          else if (blk.type === "tool-call") {
            const callId = String(blk.id ?? "");
            if (callId && !calls.has(callId)) {
              calls.set(callId, { name: blk.name ?? "", args: blk.arguments ?? "", time });
      ...
      } else if (etype === "tool/call") {
        const callId = String(data.callId ?? "");
        if (callId && !calls.has(callId)) {
          calls.set(callId, { name: String(data.name ?? ""), args: String(data.arguments ?? ""), time });
          callOrder.push(callId);
        }
      } else if (etype === "tool/result") {
        const msg = data.message ?? {};
        const blk = Array.isArray(msg.content) ? msg.content.find((b) => b && b.type === "tool-result") : void 0;
        const callId = String(blk?.toolCallId ?? "");
        const call = calls.get(callId);
        if (call) done.add(callId);
          ...
          tools: [
            {
              name: call?.name ?? "",
              args: call?.args ?? "",
              isError: blk?.isError === true || !!data.error,
        ...
        if (reason?.kind === "error") {
          turns.push({
            id: String(seq),
            role: "system",
            text: `\u56DE\u5408\u51FA\u9519\uFF1A${reason.error?.message ?? "\u672A\u77E5\u9519\u8BEF"}`,
            time
          });
        }
      } else if (etype === "assistant/chunk" || etype === "text-chunks" || etype === "reasoning-chunks" || etype.startsWith("chunkrow/")) {
        const effSeq = typeof ev.seq === "number" ? ev.seq : Number(ev.seq0 ?? 0);
        if (effSeq > lastAssistantSeq) {
          if (etype === "assistant/chunk") {
            const chunk = data.chunk ?? {};
            if (chunk.type === "text-delta") partialText += chunk.text ?? "";
            else if (chunk.type === "reasoning-delta") partialReasoning += chunk.text ?? "";
          } else if (etype === "text-chunks" || etype === "chunkrow/text-chunks") {
            partialText += (data.texts ?? []).join("");
          } else if (etype === "reasoning-chunks" || etype === "chunkrow/reasoning-chunks") {
            partialReasoning += (data.texts ?? []).join("");
          }
        }
      }
    }
    for (const callId of callOrder) {
      ...
      turns.push({
        ...
        role: "tool",
        text: call.name,
        tools: [{ name: call.name, args: call.args }],
        time: call.time
      });
    }
    ...
    for (let i = turns.length - 1; i >= 0; i--) {
      if (turns[i].role === "user") {
        lastUserTurnText = turns[i].text ?? "";
        break;
      }
    }
    for (const q of Array.isArray(sess?.queue) ? sess.queue : []) {
      if (!q || typeof q !== "object") continue;
      if (q.placement === "context") continue;
      const text = typeof q.text === "string" && q.text || q.preview || "";
      if (!text || text === lastUserTurnText) continue;
      turns.push({
        id: `q-${q.id ?? q.messageId ?? turns.length}`,
        role: "user",
        text,
        time: Date.now(),
        queued: true
    ...
    const perror = sess?.promptError?.error;
    const error = sess?.lastAgentError ?? (perror ? perror.message ?? "\u53D1\u9001/\u505C\u6B62\u5931\u8D25" : null);
    return {
      sessionId: String(sess?.sessionId ?? sessionIdFallback ?? ""),
      blank: sess?.blank === true,
      hasMore: win?.hasMore === true,
      running: sess?.running === true,
      partialText,
      partialReasoning,
      error,
      turns
  ...
  const COMPAT_ERROR = "\u5F53\u524D DSH \u7248\u672C\u8FC7\u4F4E\uFF08\u9700 \u2265 0.1.2-rc.1\uFF09\uFF1A\u4F1A\u8BDD\u5185\u5BB9\u4E8B\u4EF6\u7A97\u53E3\u4E0D\u53EF\u7528\uFF0CAI \u5BF9\u8BDD / \u4F1A\u8BDD\u4EFB\u52A1 / \u4F1A\u8BDD\u6D3B\u52A8 / \u5B50\u4EE3\u7406\u9762\u677F\u65E0\u6CD5\u8BFB\u53D6\u5B9E\u65F6\u8BB0\u5F55\uFF0C\u8BF7\u5347\u7EA7 DSH \u540E\u91CD\u8BD5\u3002";
  const compatSnapshot = (sessionId, withError) => ({
    ...EMPTY_CONV(),
    sessionId,
    compatUnsupported: true,
    error: withError ? COMPAT_ERROR : null
  });
  const getMainConversation = async () => {
    const binding = resolveMainBinding();
    const sessionId = getSessionId() ?? "";
    if (!binding && !sessionId) return EMPTY_CONV();
    if (!binding?.eventSource) return compatSnapshot(sessionId, !!sessionId);
    try {
      return normalizeFromEvents(binding.session?.getSnapshot?.(), binding.eventSource.getSnapshot(), sessionId);
    } catch {
      return compatSnapshot(sessionId, true);
    }
  };
  const sendMainConversation = async (msg) => {
    ...
    try {
      ...
      return { ok: false, error: rawError ?? "\u4F1A\u8BDD\u672A\u63A5\u53D7\u8BE5\u6D88\u606F\uFF0C\u8BF7\u91CD\u8BD5\u3002" };
    } catch (e) {
  ...
  const listWorkspaceSessions = () => {
    try {
      const list = ctx.sessions?.list?.getSnapshot?.();
      const byId = list?.byId ?? {};
      const current = list?.current ?? getSessionId() ?? "";
      const out = [];
      for (const [id, meta] of Object.entries(byId)) {
  ...
  const getHistoryConversation = async (id) => {
    if (!id) return null;
    try {
      const binding = ctx.sessions?.binding?.(id);
      if (!binding?.eventSource) return null;
      const sess = binding.session?.getSnapshot?.();
      return normalizeFromEvents(sess, binding.eventSource.getSnapshot(), id);
    } catch {
      return null;
    }
  };
  const subscribeMainConversation = (cb) => {
    ...
    const bind = () => {
      ...
      const binding = resolveMainBinding();
      const sRet = binding?.session?.subscribe?.(cb);
      if (typeof sRet === "function") innerDisposes.push(sRet);
      const eRet = binding?.eventSource?.subscribe?.(cb);
      if (typeof eRet === "function") innerDisposes.push(eRet);
    };
  ...
  link.rel = "stylesheet";
  link.href = `${apiBase}/${"assets/style-BEYoooti.css"}`;
  document.head.appendChild(link);
  const script = document.createElement("script");
  script.type = "module";
  script.src = `${apiBase}/${"assets/index-D1iSGLsI.js"}`;
