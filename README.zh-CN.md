...
- **官方终端后端**：本机 shell 的进程层（PTY 创建 / 重连 / 清理）在宿主官方 `ctx.webTerminals`
  服务可用时委托给官方实现，不可用时自动回退插件自建后端；SSH 终端始终走插件自建 SSH 通道。
...
src/
  ...
  client/                DSH 客户端桥接（TSX，注册进右侧栏，注入 Vue 产物 + 挂载）
    ...
    TabMenuBridge.tsx    右侧栏 tab 菜单追加项（新开编辑器 / 弹出浮窗）
    OfficialTerminalBridge.ts 官方终端桥：本机终端走宿主 ctx.webTerminals
    api.ts               「打开 tab」桥
  vue/                   Vue 3 + Vite 前端
    ...
    composables/
      ...
      domain/            git、gitGraph、svn、repoMenu（右键菜单单一来源）、terminalStore、
