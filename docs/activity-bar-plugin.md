...
- 注册表实现：`src/vue/stores/activityBar.ts`（编辑器的 `registerActivityView`，工作台的
  `registerWorkbenchActivityView`，以及对应的注销/列表 API）
- 面板集成：`src/vue/components/business/vscode/VSCodePane.vue` 与
  `src/vue/components/business/explorer/ExplorerPane.vue`（扩展图标渲染 + 动态挂载）
